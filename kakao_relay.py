...
# 상수
# ─────────────────────────────────────────────────────────────
VERSION = "4.4"          # 이 값이 4.4 미만이면 예전 파일입니다
...
ENV_KEYS = {
    ...
    # 종합 뉴스를 다른 대화방으로 보내고 싶을 때만 설정 (선택)
    "telegram_bot_token_news": "TELEGRAM_BOT_TOKEN_NEWS",
    "telegram_chat_id_news": "TELEGRAM_CHAT_ID_NEWS",
...
# 이 접두어로 시작하는 메일은 뉴스 대화방으로 보냅니다
NEWS_PREFIXES = ("[종합 브리핑]",)


def telegram_target(cfg, subject=""):
    """제목에 따라 (봇 토큰, 대화방 ID) 를 고릅니다.

    종합 뉴스는 별도 대화방으로 보낼 수 있습니다. 뉴스용 설정이 없으면
    기본 대화방으로 갑니다.
    """
    token = cfg.get("telegram_bot_token")
    chat = cfg.get("telegram_chat_id")
    if subject and any(p in subject for p in NEWS_PREFIXES):
        token = cfg.get("telegram_bot_token_news") or token
        chat = cfg.get("telegram_chat_id_news") or chat
    return token, chat


def _tg(token, method):
    return TELEGRAM_API % (token, method)


def telegram_ready(cfg):
    return bool(cfg.get("telegram_bot_token") and cfg.get("telegram_chat_id"))


def send_telegram_photo(cfg, path, caption="", target=None):
    """사진 1장 + 설명. 캡션은 1024자까지."""
    token, chat = target or telegram_target(cfg)
    with open(path, "rb") as fh:
        res = requests.post(_tg(token, "sendPhoto"),
                            data={"chat_id": chat,
                                  "caption": caption[:TG_CAPTION_LIMIT]},
                            files={"photo": (os.path.basename(path), fh, "image/png")},
                            timeout=60)
    if res.status_code != 200:
        raise RuntimeError("텔레그램 사진 전송 실패 [%d]: %s" % (res.status_code, res.text[:300]))
    return True


def send_telegram_album(cfg, paths, caption="", target=None):
    """사진 여러 장을 하나의 앨범으로. 좌우로 넘겨 봅니다. 최대 10장."""
    token, chat = target or telegram_target(cfg)
    paths = list(paths)[:10]
    if len(paths) == 1:
        return send_telegram_photo(cfg, paths[0], caption, target=target)
    ...
    try:
        for i, p in enumerate(paths):
            ...
            if i == 0 and caption:
                item["caption"] = caption[:TG_CAPTION_LIMIT]
            media.append(item)
            fh = open(p, "rb")
            handles.append(fh)
            files[key] = (os.path.basename(p), fh, "image/png")
        res = requests.post(_tg(token, "sendMediaGroup"),
        ...
        for fh in handles:
            try:
                ...
                pass
    if res.status_code != 200:
        raise RuntimeError("텔레그램 앨범 전송 실패 [%d]: %s"
                           % (res.status_code, res.text[:300]))
    return len(paths)


def send_telegram_text(cfg, text, target=None):
    """긴 글도 4096자까지 한 번에. 넘치면 나눠 보냅니다."""
    token, chat = target or telegram_target(cfg)
    chunks, buf = [], ""
    for line in str(text).split("\n"):
        if len(buf) + len(line) + 1 > TG_TEXT_LIMIT:
    ...
    for i, part in enumerate(chunks):
        res = requests.post(_tg(token, "sendMessage"),
                            data={"chat_id": chat,
                                  "text": part,
                                  "disable_web_page_preview": "true"},
                            timeout=20)
        if res.status_code != 200:
            raise RuntimeError("텔레그램 전송 실패 [%d]: %s" % (res.status_code, res.text[:300]))
        if i < len(chunks) - 1:
            time.sleep(0.5)
    return len(chunks)
...
def _pick_channels(cfg):
    """어디로 보낼지 결정합니다.

    RELAY_CHANNELS 환경변수로 강제할 수 있습니다 (예: "telegram" 또는 "telegram,kakao").
    지정이 없으면 설정된 채널을 모두 씁니다.
    """
    forced = os.environ.get("RELAY_CHANNELS", "").strip().lower()
    if forced:
        want = [c.strip() for c in forced.split(",") if c.strip()]
    else:
        want = ["telegram", "kakao"]

    out = []
    if "telegram" in want and telegram_ready(cfg):
        out.append("telegram")
    if "kakao" in want and cfg.get("kakao_rest_api_key"):
        out.append("kakao")
    if not out:
        raise RuntimeError("보낼 채널이 없습니다. 텔레그램 또는 카카오 설정을 확인하세요.")
    return out


def _render_card_file(cfg, card):
    """카드 이미지를 그려 경로 목록을 돌려줍니다. 실패하면 빈 목록.

    카드 렌더러가 화면 비율에 맞춰 여러 쪽으로 나누므로 목록입니다.
    """
    if not card:
        return []
    try:
        ...
        log("card_render.py 를 불러오지 못했습니다 (%s). 'pip install pillow' 필요." % e)
        return []
    try:
        path = os.path.join(BASE_DIR, "card.png")
        log("카드 이미지를 그리는 중...")
        out = render_card(card, path)
        paths = [out] if isinstance(out, str) else list(out or [])
        paths = [p for p in paths if p and os.path.exists(p)]
        if not paths:
            paths = [path] if os.path.exists(path) else []
        log("  %d쪽 / 합계 %d KB"
            % (len(paths), sum(os.path.getsize(p) for p in paths) // 1024))
        for i, p in enumerate(paths):
            _archive_card(card, p, i + 1 if len(paths) > 1 else 0)
        return paths
    except Exception as e:
        log("카드 렌더 실패: %s" % e)
        return []
...
def _archive_card(card, path, page=0):
    """cards 폴더에 날짜별 사본을 남깁니다."""
    try:
        cards_dir = os.path.join(BASE_DIR, "cards")
        os.makedirs(cards_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
...
def _send_telegram(cfg, card, subject, text):
    """텔레그램: 사진 + 전체 요약을 한 번에. 카카오와 달리 쪼갤 필요가 없습니다."""
    target = telegram_target(cfg, subject)
    where = "뉴스방" if target[1] != cfg.get("telegram_chat_id") else "기본방"
    paths = _render_card_file(cfg, card)
    parts = []
    if paths:
        # 캡션이 1024자를 넘으면 사진은 짧은 제목만 달고 본문은 따로 보냅니다
        cap = text if len(text) <= TG_CAPTION_LIMIT else (card.get("title") or subject)
        send_telegram_album(cfg, paths, cap, target=target)
        parts.append("사진 %d건" % len(paths))
        if cap is not text:
            n = send_telegram_text(cfg, text, target=target)
            parts.append("텍스트 %d건" % n)
    else:
        n = send_telegram_text(cfg, text, target=target)
        parts.append("텍스트 %d건" % n)
    return "텔레그램(%s) %s" % (where, " + ".join(parts))


def _send_kakao_all(cfg, card, subject, text, link):
    """카카오: 사진(feed) + 200자 단위로 쪼갠 요약."""
    sent_img = False
    if card:
        try:
            sent_img = _send_card(cfg, card, subject, link)
        except Exception as ce:
            log("카카오 사진 전송 실패(텍스트로 대체): %s" % ce)
    n = 0
    if (not sent_img) or cfg.get("send_text_with_image", True):
        n = send_kakao(cfg, text, link=link)
    return "카카오 " + " + ".join(
        x for x in (("사진 1건" if sent_img else ""), ("텍스트 %d건" % n if n else "")) if x)


def cmd_run(force=False, latest_only=False):
    log("릴레이 v%s%s" % (VERSION, " · 재전송 모드" if force else ""))
    try:
        import card_render
        log("카드 렌더러 폭 %d · 쪽높이 %d · 배율 %s"
            % (card_render.W, getattr(card_render, "PAGE_H", 0), card_render.SCALE))
    except Exception:
        log("카드 렌더러를 불러오지 못했습니다 (텍스트만 전송됩니다)")
    cfg = load_config()
    state = load_json(STATE_PATH, {"sent": []}) or {"sent": []}

    # 카카오만 쓰는 경우에만 토큰을 미리 확인합니다
    ...
    channels = _pick_channels(cfg)
    log("전송 채널: %s" % ", ".join(channels))
    ...
    for uid, subject, text, msgid, link, card in briefs:
        ...
        try:
            ...
            if "telegram" in channels:
                results.append(_send_telegram(cfg, card, subject, text))
            if "kakao" in channels:
                results.append(_send_kakao_all(cfg, card, subject, text, link))
            log("전송 완료 [%s] → %s" % (subject, " / ".join(r for r in results if r)))
