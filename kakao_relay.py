...
# 상수
# ─────────────────────────────────────────────────────────────
VERSION = "4.0"          # 이 값이 4.0 미만이면 예전 파일입니다
...
def _tg(cfg, method):
    return TELEGRAM_API % (cfg["telegram_bot_token"], method)
...
def send_telegram_photo(cfg, path, caption=""):
    """사진 1장 + 설명. 캡션은 1024자까지."""
    with open(path, "rb") as fh:
        res = requests.post(_tg(cfg, "sendPhoto"),
...
def send_telegram_text(cfg, text):
    ...
    for i, part in enumerate(chunks):
        res = requests.post(_tg(cfg, "sendMessage"),
                            data={"chat_id": cfg["telegram_chat_id"],
                                  "text": part,
...
def _send_telegram(cfg, card, subject, text):
    ...
    if path:
        ...
        send_telegram_photo(cfg, path, cap)
        parts.append("사진 1건")
        if cap is not text:
            n = send_telegram_text(cfg, text)
            parts.append("텍스트 %d건" % n)
    else:
        n = send_telegram_text(cfg, text)
        parts.append("텍스트 %d건" % n)
    return "텔레그램 " + " + ".join(parts)
