...
def render_card(data, out_path="card.png"):
    ...
    if holdings:
        ...
        for h in holdings:
            ...
            rl = _wrap(d, str(h.get("reason") or ""), f(10.5), inner)[:2] if h.get("reason") else []
            has_line = bool(h.get("opinion")) or bool(parts)
            rh = 38 + (21 if has_line else 0) + 15 * len(rl) + 11
            rows.append((h, parts, rl, has_line, rh))
        box_h = 12 + sum(r[4] for r in rows)
        card_box(y, box_h)
        ry = y + 11
        for i, (h, parts, rl, has_line, rh) in enumerate(rows):
            col = _hex(h.get("color"))
            d.rounded_rectangle([PAD + 14, ry + 5, PAD + 23, ry + 14], 3, fill=col)
            nx = PAD + 31
            nm = str(h.get("name", ""))
            d.text((nx, ry + 1), nm, font=f(13, True), fill=INK)
            nx += d.textlength(nm, font=f(13, True)) + 6
            if h.get("qty"):
            ...
            if has_line:
                ...
                if op:
                    ...
                    ow = d.textlength(str(op), font=f(11, True)) + 20
                    d.rounded_rectangle([bx, ly, bx + ow, ly + 19], 9, fill=_tint(ocol, 0.15))
                    d.text((bx + 10, ly + 2), str(op), font=f(11, True), fill=ocol)
                    bx += ow + 10
                if parts:
                    d.text((bx, ly + 3), "  ·  ".join(parts), font=f(11, True), fill=INK)
                ly += 21
            for ln in rl:
                d.text((PAD + 31, ly), ln, font=f(10.5), fill=MUTED)
                ly += 15
            if i < len(rows) - 1:
                d.line([PAD + 14, ry + rh - 6, W - PAD - 14, ry + rh - 6], fill=LINE)
            ry += rh
        y += box_h
    ...
    if watch:
        ...
        rows = []
        for wt in watch:
            tgt = "  ·  ".join(str(x) for x in (wt.get("target_s"), wt.get("target_m")) if x)
            nl = _wrap(d, str(wt.get("note") or ""), f(10.5), inner)[:2] if wt.get("note") else []
            rows.append((wt, tgt, nl, 21 + (18 if tgt else 0) + 15 * len(nl) + 12))
        box_h = 12 + sum(r[3] for r in rows)
        card_box(y, box_h)
        ry = y + 11
        for i, (wt, tgt, nl, rh) in enumerate(rows):
            mk = str(wt.get("market") or "").upper()
            dom = mk.startswith("KR") or "국내" in str(wt.get("market") or "")
            mcol = _hex("#2E6FD9") if dom else _hex("#158A6E")
            lbl = "국내" if dom else "미국"
            d.rounded_rectangle([px, ry + 1, px + pw, ry + 19], 8, fill=_tint(mcol, 0.16))
            d.text((px + (pw - d.textlength(lbl, font=f(10, True))) / 2.0, ry + 3), lbl,
                   font=f(10, True), fill=mcol)
            nx = tx0
            nm = str(wt.get("name", ""))
            d.text((nx, ry + 1), nm, font=f(13, True), fill=INK)
            nx += d.textlength(nm, font=f(13, True)) + 7
            if wt.get("ticker"):
                d.text((nx, ry + 4), str(wt["ticker"]), font=f(10), fill=MUTED)
            if wt.get("price"):
                pt = str(wt["price"])
                d.text((W - PAD - 14 - d.textlength(pt, font=f(12, True)), ry + 2), pt,
                       font=f(12, True), fill=INK)
            ty2 = ry + 22
            if tgt:
                d.text((tx0, ty2), tgt, font=f(11, True), fill=_hex("#158A6E"))
                ty2 += 18
            for ln in nl:
                d.text((tx0, ty2), ln, font=f(10.5), fill=MUTED)
                ty2 += 15
            if i < len(rows) - 1:
                d.line([px, ry + rh - 6, W - PAD - 14, ry + rh - 6], fill=LINE)
            ry += rh
        y += box_h
