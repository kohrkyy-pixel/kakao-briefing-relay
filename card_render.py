...
INK = (22, 32, 46)
SUB = (74, 90, 110)
MUTED = (100, 114, 130)   # 흰 배경 대비 4.92:1 (WCAG AA 통과)
LINE = (227, 232, 239)
PAPER = (245, 247, 250)
WHITE = (255, 255, 255)
UP = (210, 69, 60)
...
PAD = 16
SCALE = 2.5      # 실제 출력 1400px — QHD+(1440) 화면에서도 선명
...
def render_news_card(data, out_path="card.png"):
    ...
    for ln in _clip(_wrap(d, foot, f(11.5), iw), 3):
        d.text((PAD, y), ln, font=f(11.5), fill=MUTED)
        y += 17
    y += 16
...
def render_card(data, out_path="card.png"):
    ...
    if holdings:
        ...
        for h in holdings:
            ...
            rl = _clip(_wrap(d, str(h.get("reason") or ""), f(12.5), tw), 3) if h.get("reason") else []
            op = h.get("opinion")
        ...
        for i, (h, parts, rl, op, rh) in enumerate(rows):
            ...
            if h.get("qty"):
                d.text((nx, ry + 4), "%s주" % h["qty"], font=f(12), fill=MUTED)
            hp = h.get("pnl")
            if hp is not None:
            ...
            if rl:
                ly += 4
                for ln in rl:
                    d.text((tx + 15, ly), ln, font=f(12.5), fill=SUB)
                    ly += 17
            if i < len(rows) - 1:
    ...
    if watch:
        ...
        for wt in watch:
            ...
            nl = _clip(_wrap(d, str(wt.get("note") or ""), f(12.5), W - PAD - 16 - px), 3) \
                if wt.get("note") else []
        ...
        for i, (wt, tg, nl, rh) in enumerate(rows):
            ...
            if wt.get("ticker"):
                d.text((nx, ry + 5), str(wt["ticker"]), font=f(12), fill=MUTED)
            if wt.get("price"):
                pt = str(wt["price"])
            ...
            if nl:
                ty += 5
                for ln in nl:
                    d.text((px, ty), ln, font=f(12.5), fill=SUB)
                    ty += 17
            if i < len(rows) - 1:
    ...
    for ln in _clip(_wrap(d, foot, f(11.5), iw), 4):
        d.text((PAD, y), ln, font=f(11.5), fill=MUTED); y += 17
    y += 16
    return _finish(img, y, out_path)
